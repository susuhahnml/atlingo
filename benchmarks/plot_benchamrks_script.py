...
def clean_df(df):
    df.drop(df.columns[-1*N_OUT:], axis=1, inplace=True) #Drop min
    df.drop(df.columns[-2*N_OUT:-1*N_OUT], axis=1, inplace=True) #Drop max
    ...
    # print(new_cols[-2*N_OUT]+new_cols[-1*N_OUT])
    df.columns = new_cols
